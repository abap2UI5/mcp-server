 ...
 * mid-task, which had to pull the whole interface through its context to answer
...
/** The interface parsed, mtime-cached (lib/cache.mjs): `{ raw, parsed }`, or
 *  null when the checkout does not carry it. The parse is pure over the file
 *  text, so caching it per file version IS the live-read contract - an
 *  unchanged interface costs a stat, a pulled one re-parses itself. */
export function readApiParsed() {
  const file = apiFile();
  if (!file || !fs.existsSync(file)) return null;
  return readCached(file, (raw) => ({ raw, parsed: parseApi(raw) }));
}
