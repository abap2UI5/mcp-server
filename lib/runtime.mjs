...
import { isRemoteCheckout } from './remote.mjs';
...
async function lintOnce(className, { signal, onLine } = {}) {
  ...
  try {
    spawned = await spawnWithTimeout(
      ...
      { cwd: corpus(), timeoutMs: timeoutOf('A2UI5_MCP_LINT_TIMEOUT_MS'), signal, onLine },
    );
  } finally {
...
export function buildBackend({ onLine, mode = 'auto', signal } = {}) {
  ...
  let effective = mode;
  if (mode === 'auto') effective = canIncrement ? 'incremental' : (a2 ? 'prebuilt' : 'full');
  ...
  if ((incremental || prebuilt) && !a2) {
    return Promise.resolve({ ok: false, code: 1, tail: 'abap2UI5 checkout not found — clone https://github.com/abap2UI5/abap2UI5 as a sibling of mcp-server (or run `npm run node:setup` in samples-controls), or point A2UI5_HOME at an existing checkout; then run build_backend again (mode prebuilt needs nothing else)' });
  }
  ...
  const run = (async () => {
    ...
    try {
      ...
      if (incremental) {
        // the deployed dev apps: only a local corpus has a sandbox to copy from
        if (corpusDir && fs.existsSync(devDir())) {
