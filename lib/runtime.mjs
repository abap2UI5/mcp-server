...
import { resolveA2UI5, resolveSamplesControls, SERVER_ROOT } from './repos.mjs';
    ...
    try {
      ...
      if (incremental) {
        ...
        if (!fs.existsSync(path.join(lib, 'src'))) {
          ...
          execSync(`git clone --quiet --depth=1 https://github.com/open-abap/open-abap-core ${lib}`, { cwd: a2, timeout: buildTimeout });
          execSync(`node ${path.join(corpus(), "web/ci/patch_open_abap_xml.mjs")} ${lib}`, { cwd: a2, timeout: buildTimeout });
        }
