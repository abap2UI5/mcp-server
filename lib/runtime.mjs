 ...
 *                      prebuilt when the abap2UI5 checkout is there or can be
 *                      cloned - and when the release carries no asset (HTTP
 *                      404), transpile: the framework's own few-minute build,
 *                      announced in the log, never the tens-of-minutes corpus
 *                      build; full only when there is no abap2UI5 checkout to
 *                      unpack into and the corpus can bootstrap one.
 */
...
export function buildBackend({ onLine, mode = 'auto', signal } = {}) {
  ...
  if ((incremental || transpile || (prebuilt && !canClone)) && !a2) {
...
export async function transpileFramework({ a2, onLine = () => {}, signal, timeoutMs = 30 * 60_000 } = {}) {
  ...
  if (!fs.existsSync(path.join(a2, 'node_modules', '@abaplint', 'transpiler-cli'))) {
    steps.push({ what: 'npm ci', cmd: 'npm', args: ['ci', '--no-audit', '--no-fund'] });
  }
  steps.push({ what: 'npm run downport', cmd: 'npm', args: ['run', 'downport'] });
  steps.push({ what: 'npm run auto_transpile', cmd: 'npm', args: ['run', 'auto_transpile'] });
  ...
  for (const step of steps) {
    ...
    const res = await spawnWithTimeout(step.cmd, step.args, { cwd: a2, env, timeoutMs, onLine, signal });
...
export async function downloadPrebuilt({ a2, onLine = () => {}, signal, timeoutMs = 30 * 60_000, fetchImpl = globalThis.fetch } = {}) {
  ...
  try {
     ...
     * the cleanup below can remove it (Windows refuses an open file). */
    if (out) out.destroy();
...
export async function runUnitTests({ className, classNames, signal, onLine } = {}) {
  const a2 = a2Local();
  ...
  if (wanted.length) {
    ...
    if (patched) {
      tmpEntry = path.join(a2, 'node/output', `index-mcp-${(cls || 'selection').toLowerCase()}.mjs`);
