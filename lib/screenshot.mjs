 ...
 */
export async function screenshotSource({ screenshotFiles, abapSource, xml, sizes, theme, model, onProgress, renderer }) {
  ...
  try {
    ...
    return await screenshotFiles([file], {
      ...
      // an already-open renderer (the warm path, lib/renderer.mjs) - the
      // linter uses it and does not close it; older linters ignore the option
      // and open their own, which is why the caller feature-detects first
      ...(renderer ? { renderer } : {}),
    });
  } finally {
