...
export function parseCapabilities(rawText = null) {
  // the file parse is mtime-cached (lib/cache.mjs): still re-read the moment
  // the checkout changes, one stat instead of a parse while it does not
  if (rawText === null) return readCached(capabilitiesFile(), (text) => parseEntries(text));
  return parseEntries(rawText);
}
