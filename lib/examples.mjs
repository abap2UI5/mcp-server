...
function readCatalogue(c) {
  const readMd = () => {
    if (!c.mdFile) return [];
    try { return parseRows(fs.readFileSync(c.mdFile, 'utf8'), c.repo); } catch { return []; }
  };
  if (c.source !== 'catalogue.json') return readMd();
  let fromJson = null;
  try { fromJson = catalogueEntries(JSON.parse(fs.readFileSync(c.file, 'utf8')), c.repo); } catch { fromJson = null; }
