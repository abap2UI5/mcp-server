 ...
 * discover from the tool descriptions alone that app_guide comes first and
 * run_app comes last.
 *
...
1. ORIENT. Call \`examples\` with keywords from the task: what comes back is a complete, gated
   app worth more than any snippet - \`read_example\` hands you its source, checkout or not. Call \`capabilities\` before
...
5. PROVE IT RUNS. \`deploy_app\` (with its testclasses argument when the app has logic worth asserting - a
   local z2ui5_if_client double, see app_guide chapter 9), then \`build_backend\` (the first time
   it downloads the framework's prebuilt backend, a minute; a full build is tens of minutes and
   normal, not a hang), then \`run_app\`: read the status, the real page errors and the
   screenshot of the running app. \`interact_app\` then drives the event branch - click the
   button, fill the input, look again - and \`run_unit_tests\` runs the deployed tests. Iterate;
   \`remove_app\` cleans up when you are done. \`verify_app\` runs all of that in one call and
   stops at the first stage that fails - the end-of-iteration check.
...
5. PROVE IT. \`deploy_app\`, \`build_backend\` (prebuilt in a minute the first time, tens of
   minutes for a full build), then \`run_app\` and compare the running app against the
   original; \`interact_app\` drives the sample's interactions the way the demo kit shows them. Call \`pitfalls\` before
 ...
 *
 * This used to be a two-way ternary on `name === 'build-an-abap2ui5-app'`,
