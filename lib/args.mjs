 ...
 */
export function stringArray(value, { name, maxItems = 50, maxLength = 200 }) {
  if (!Array.isArray(value)) {
    throw new Error(`${name} must be an array of strings, not ${JSON.stringify(value)?.slice(0, 80)} — e.g. ["sap.m.Wizard"]`);
  }
  if (!value.length) {
    throw new Error(`${name} is empty — pass at least one entry, e.g. ["sap.m.Wizard"]`);
  }
