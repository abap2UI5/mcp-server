...
export const TOOLS = [
  {
    ...
    description:
      ...
      + '(and the env var or clone that fixes it), which dev sandbox deploy_app writes into (the corpus, or the '
      + 'framework checkout when there is no corpus) and what is deployed there, whether the backend is built, '
      + 'prebuilt and running, whether the unit-test runner exists, where a framework clone would land, and '
      + 'whether git, tar, npx and a Chromium are on this machine. Reads only; changes nothing.',
  ...
  },
  {
    ...
    description:
      ...
      + 'complete, gated app: read it whole, then take the pattern, not the file.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      ...
      'dev sandbox - samples-controls\' src/zz_dev/, or the abap2UI5 checkout\'s node/zz_dev/ when there is no ' +
      'corpus - and lints it (the corpus\' abaplint config, or app-template\'s with the framework sources as ' +
      'the dependency: the lint a real project runs). The class must implement ' +
  ...
  },
  {
    ...
    description:
      ...
      + 'an edit that never arrived; deploy_app writes, remove_app lists and deletes.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      ...
      'incremental when a prior build exists: only src/zz_dev/ is re-copied and re-transpiled (~1-2 min); with no ' +
      'prior build it is prebuilt. mode prebuilt downloads the backend the framework\'s release workflow built ' +
      '(backend-<version>.tar.gz on the GitHub release of the checkout\'s version) and unpacks it — a minute, and ' +
      'it needs only the abap2UI5 checkout, not samples-controls; the framework without the corpus\' ports, which ' +
      'is what an app of your own needs. mode full runs the complete e2e-build (downport + transpile, tens of ' +
      'minutes, needs samples-controls) — for a checkout on an unreleased commit, when framework/port sources ' +
      'changed, or when the incremental transpile rejects a construct (then simplify the ABAP or go full). ' +
      'mode transpile runs the framework\'s own build in its checkout (npm run downport + auto_transpile, a few minutes, ' +
      'no corpus) — what auto falls back to when the release carries no asset yet. A failed download is otherwise ' +
      'reported, never silently turned into a tens-of-minutes full build. Stops a running backend first. Only one build runs at a time: a second call with the ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        mode: { type: 'string', enum: ['auto', 'incremental', 'prebuilt', 'transpile', 'full'], description: 'default auto' },
      },
    },
  },
  {
    ...
    description:
      ...
      + 'are the paging cursor). The log survives a server restart. Nothing to read until a '
      + 'build has run.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      ...
      + 'only that object\'s test classes run (the generated runner is filtered on it); without it every test of '
      + 'the transpiled tree runs, the framework\'s own included, which takes minutes. A test that passes here '
  ...
  },
  {
    ...
    description:
      ...
      + 'stage\'s result under `stages`, `ok`, and `stoppedAt` naming the stage that failed - so one call answers '
